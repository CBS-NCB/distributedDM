...
kmap_horI  = round(interp2(xdom,ydom,kmap_horS,xdomI,ydomI,'spline')); % all were 'nearest' originally MA 20150724
...
imI = round(interp2(xdom,ydom,im,xdomI,ydomI,'spline')); %interpolate to be the same size as the maps
...
for i = 1:length(labdom)-1
    ...
    %figure;imshow(spCov{i},[]);title('Spatial Visual Coverage'); % MA 20150724
        ...
        if AreaSign(i)*AreaSign(j) == 1 %if they have the same sign
            ...
            if touchflag %if they border each other
                ...
                if OLap < .1 %If there is very little visual overlap (i.e. not redundant), fuse them
                    ...
                    figure,
                    subplot(2,2,1), 
                    ploteccmapfuse((patch1+patch2).*kmap_hor,[min(kmap_hor(find(patch1+patch2))) max(kmap_hor(find(patch1+patch2)))],pixpermm);
                    subplot(2,2,2), 
                    ploteccmapfuse((patch1+patch2).*kmap_vert,[min(kmap_vert(find(patch1+patch2))) max(kmap_vert(find(patch1+patch2)))],pixpermm);
                    subplot(2,2,3), 
