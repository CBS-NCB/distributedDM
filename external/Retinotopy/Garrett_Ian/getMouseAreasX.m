function [im, ptch]= getMouseAreasX(kmap_hor,kmap_vert,pixpermm, scales)
...
figure(10), clf; subplot(3,4,1)
imagesc(xdom,ydom,kmap_hor,[min(kmap_hor(:)) max(kmap_hor(:))]);%[-50 50]
axis image; colorbar
title('1. Horizontal (azim deg)')

subplot(3,4,2)
imagesc(xdom,ydom,kmap_vert,[min(kmap_vert(:)) max(kmap_vert(:))]),
axis image;  colorbar
...
global extraParams;
SE = strel('disk',extraParams.disksize, 0); % this was 2 originally, instead of extraParams.disksize; MA
...
%save M150714_3_PatcgSgn.mat patchSign
ptch = patchSign; % MA

%% Plot eccentricity map, with [0 0] defined as V1's center-of-mass

SE = strel('disk',10); % originally disk size was 10
imdum = imopen(imseg,SE); % imseg is segmented VFS
[CoMxy, Axisxy] = getPatchCoM(imdum); % CoMxy, coordinates of Center of Mass

V1id = getV1id(imdum);

% figure; imshow(bwlabel(imdum,4),[]);
% hold on; text(round(CoMxy(V1id,1)),round(CoMxy(V1id,2)),'V1','color','red')
...
az = (kmap_hor - AreaInfo.Vcent(1))*pi/180; %azimuth in radian. MA: eq5 Garrett 2014=> (H-<H>)
alt = (kmap_vert - AreaInfo.Vcent(2))*pi/180; %altitude in radian. MA: eq5 Garrett 2014=> (V-<V>)
AreaInfo.kmap_rad = atan(  sqrt( tan(az).^2 + (tan(alt).^2)./(cos(az).^2)  )  )*180/pi;  %Eccentricity in °. MA: eq5 Garrett 2014

figure(10); subplot(3,4,8)
...
%%
%save AreaContours.mat im %MA 20150713
