function [kmap_hor, kmap_vert, delay_hor, delay_vert, magS] = Gprocesskret(f1,bw,adaptbit,hl,hh)

%{
f1: Each cell is the "complex valued, f1 map" for the four directions of the drifting bar 
(i.e. each pixel is the FFT at the stimulus frequency).  
f{1} is computed from bar sweeping to the right. 
f{2} is from bar sweeping up, f{3} to the left, f{4} down. 
bw is the region of interest... 1's and 0's. 
adaptbit: Adaptive smoothing.  This never seemed to work for me.  Set to 0 if you don't want it.
hl/hh: low pass and high pass smoothing kernel. Set them to [ ] if you don't want to use it.
%}
...
if adaptbit && ~isempty(hl)  %if we want to use adaptive smoothing

    ang1 = adaptiveSmoother(ang1,hl);
    ang3 = adaptiveSmoother(ang3,hl);
    ang0 = adaptiveSmoother(ang0,hl);
    ang2 = adaptiveSmoother(ang2,hl);
...
%     win = [3 3];
%     ang1 = wiener2(ang1,win);
%     ang3 = wiener2(ang3,win);
%     ang0 = wiener2(ang0,win);
%     ang2 = wiener2(ang2,win);
elseif ~isempty(hl) %else use standard smoothing
