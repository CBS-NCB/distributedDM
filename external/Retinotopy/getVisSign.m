function [VSgnMap, VS_Thr] = getVisSign(map_horz, map_vert, reszfact)
% INPUTS:
...
xdom = (0:size(map_horz,2)-1)*pcopxsz/reszfact;
ydom = (0:size(map_horz,1)-1)*pcopxsz/reszfact;
...
GF = fspecial('gaussian',size(map_horz), 10); % Gaussian Sigma = 10
GF = GF/sum(GF(:));
...
figure, clf
subplot(2,2,1)
imagesc(xdom, ydom, map_horz); % [min(map_horz(:)) max(map_horz(:))]
