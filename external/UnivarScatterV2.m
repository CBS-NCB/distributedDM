function [xPositions, yPositions, Label, RangeCut, hList, p12] = UnivarScatterV2(data, varargin)
%--------------------------------------------------------------------------
...
%
%                       +'XPositions' - Vector with the same number of columns as the data specifying their X position in the graph
%
...
%                       your data.
%          hList      - List of handles for the scatter points
%
% version: <1.00> from 30/11/2015
% 
%       Manuel Lera Ram�rez: manulera14@gmail.com
%
%       Developed during Master Thesis Internship in Marcos Gonz�lez-Gait�n's Lab
%       Departments of Biochemistry and Molecular Biology, Sciences II, 
%       30 Quai Ernest-Ansermet, CH-1211 Geneva 4, Switzerland
%
%       V2 Modified by Javier G. Orlandi for NETCAL
%       - Added option to specify original X values
%--------------------------------------------------------------------------
...
if ~isempty(Ind)
    ...
    if ischar(valueVars{Ind}) && any([strcmp('pointbar',valueVars{Ind}), strcmp('box',valueVars{Ind}),strcmp('none',valueVars{Ind}),strcmp('CI',valueVars{Ind}),strcmp('lines',valueVars{Ind})]) 
...
Ind=strmatch('XPositions',stringVars);
if ~isempty(Ind)
    %Check that XPositions is a vector of the correct size
    if isnumeric(valueVars{Ind}) && all(size(valueVars{Ind})== [1,size(data,2)])
        XPositionsOrigin=valueVars{Ind};
    else
        error(sprintf('wrong XPositions, should be a vector with the same columns as the original data'))
    end
else
  try
    XPositionsOrigin = 1:size(data, 2);
  catch
    XPositionsOrigin = [];
  end
end
...
if ~isempty(Ind)
    ...
    %MarkerEdgeColor='k';
    MarkerEdgeColor = lines(size(data, 2))*0.5;
    EdgeIsArray = true;
end
...
if ~isempty(Ind)
    ...
    MarkerFaceColor = lines(size(data, 2));
    FaceIsArray = true;
end
...
elseif strcmp(Whiskers,'box')
    %SEMColor=[1 1 1]*0.95;
    SEMColor = MarkerFaceColor*1;
    SEMColor(SEMColor > 1) = 1;
    SEMColorIsArray = true;
else
    SEMColor='k';
end
...
elseif strcmp(Whiskers,'box')
    %StdColor=[1 1 1]*0.85;
    StdColor = MarkerFaceColor*1;
    StdColor(StdColor > 1) = 1;
    StdColorIsArray = true;
elseif strcmp(Whiskers,'CI')
    %StdColor=[1 1 1]*0.85;
    StdColor = MarkerFaceColor*1;
    StdColor(StdColor > 1) = 1;
    StdColorIsArray = true;
...
% overlap.
if(isempty(XPositionsOrigin))
  xValues=ones(size(yValues))*i;
else
  xValues=ones(size(yValues))*XPositionsOrigin(i);
end
...
if strcmp(Whiskers,'box')
    ...
    %h = rectangle('Position',[XPositionsOrigin(i)-Width/WhiskersWidthRatio,yMean-yStd,2*Width/WhiskersWidthRatio,2*yStd ],'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
    p1 = patch('XData', [0 1 1 0]*2*Width/WhiskersWidthRatio+XPositionsOrigin(i)-Width/WhiskersWidthRatio, ...
      'YData', [0 0 1 1]*2*yStd+yMean-yStd, 'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
          %'YData', [0 0 1 1]*2*yStd+yMean-yStd, 'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1, 'FaceAlpha', 0.25, 'EdgeAlpha', 0.25);
    %plot the mean+-SEM box
    %h = rectangle('Position',[XPositionsOrigin(i)-Width/WhiskersWidthRatio,yMean-yCI,2*Width/WhiskersWidthRatio,2*yCI ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
    p2 = patch('XData', [0 1 1 0]*2*Width/WhiskersWidthRatio+XPositionsOrigin(i)-Width/WhiskersWidthRatio, ...
      'YData', [0 0 1 1]*2*yCI+yMean-yCI, 'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
          %'YData', [0 0 1 1]*2*yCI+yMean-yCI, 'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1, 'FaceAlpha', 0.15, 'EdgeAlpha', 0.15);
    %plot the mean line 
    % Manual alpha-like feature
    alphaColor1 = StdColor(StdIndex,:)*0.2+0.8;
    alphaColor1(alphaColor1 > 1) = 1;
    alphaColor2 = StdColor(StdIndex,:)*0.35+0.65;
    ...
    plot([XPositionsOrigin(i)-Width/WhiskersWidthRatio XPositionsOrigin(i)+Width/WhiskersWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex,:),'LineWidth',2)
    p12 = [p1; p2];
else
  p12 = [];
end
...
%cuts = abs(max(yValues)-min(yValues))/RangeCut(i)
  ...
  curIt = curIt + 1;
    ...
    if all(sum(subsetind)~=[1 0]) %If there's only one point, we represent it in the middle, and xValues remains equal to i
        ...
        if(isempty(XPositionsOrigin))
          ...
          xSubset=ones(sum(subsetind),1)*XPositionsOrigin(i);
        end
...
h = scatter(xValues,yValues,PointSize,PointStyle,'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:),'MarkerFaceColor',MarkerFaceColor(FaceIndex,:),'LineWidth',LineWidth);
hList = [hList; h];
...
if strcmp(Whiskers,'pointbar')
    %plot the mean line
    yMean=mean(yValues);
    yStd=std(yValues);
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;
    hp = plot(i,yMean, 'o', 'MarkerSize', 4, 'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:), 'MarkerFaceColor',MarkerFaceColor(FaceIndex,:));
    uistack(hp, 'bottom');
    hp = plot(i*[1 1], yMean + [-1 1]*ySem, '-', 'Color', MarkerEdgeColor(EdgeIndex,:), 'LineWidth', 1.5);
    uistack(hp, 'bottom');
    h.MarkerFaceColor = h.MarkerFaceColor*0.75;
    h.MarkerEdgeColor = h.MarkerEdgeColor*0.75;
end

xPositions(yValues_nanIndex,i)=xValues(x_SortingIndex);
end
set(gca,'xtick',XPositionsOrigin(1:size(data,2)))
