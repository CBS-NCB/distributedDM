function [pvalQ, pvalZ, vSimIndex, ClustIndData] = KSigClustPK(data,K,paramstruct) 
%
% KSigClustPK, multi-cluster statistical SIGnificance of CLUSTers
%   Patrick Kimes' matlab function extending 'SigClustSM.m'.
%     - implementation outputs p-value for H0: K = 1, H1: K = k
%       using k-means clustering index
%
%   Started:        09/23/2012
...
%    fields           values
%
%    vclass           Vector of 1's to K's, which indicates given class labels 
...
%
%    twoMtype         Type of K-means clusterings to compute
%                     1 - use simple random restarts
%                           (i.e. make calls to SigClustKmeanRepPK.m)
%                     2 (default) - use PCA starts
%                           (i.e. make calls to SigClustKmeanFastPK.m)
%
%    twoMsteps        Number of steps to use in K-means clustering
%                         computations
...
%                     Suggested uses:
%                       For vclass = 0:
%                         legendcellstr = {{'Testing Best K-means Split'}}
%                       For given vclass:
...
% Outputs:
%
%    pvalQ            (K-1) x 1 vector of Simulated SigClust P-values,
%                     Based on Empirical Quantiles, 
%                     Computed by cquantSM.m
%
%    pvalZ            (K-1) x 1 vector of Simulated SigClust P-values,
...
%     BGSDsavestr = getfield(paramstruct,'BGSDsavestr') ; 
    ...
    if ~(ischar(BGSDsavestr) || isempty(BGSDsavestr)) ;    %  then invalid input, so give warning
      disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
      disp('!!!   Warning from MCSigClustPK.m:   !!!') ;
...
if vclass ~= 0 ;
  if  (size(vclass,1) ~= 1)  ||  (size(vclass,2) ~= n)  ;
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    disp('!!!   Error from MCSigClustPK.m:        !!!') ;
  ...
  if sum(sum(flags)) ~= n ; 
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    disp('!!!   Error from MCSigClustPK.m:              !!!') ;
    disp('!!!   vclass needs to contain all 1s ... Ks   !!!') ;
    disp('!!!   Terminating Execution                   !!!') ;
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    return ;
  end ;
  if numel(unique(vclass)) ~= max(vclass) ;
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    disp('!!!   Error from MCSigClustPK.m:              !!!') ;
    disp('!!!   vclass needs to contain all 1s ... Ks   !!!') ;
    disp('!!!   Terminating Execution                   !!!') ;
  ...
  if max(vclass) ~= K ;
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    disp('!!!   Error from MCSigClustPK.m:              !!!') ; 
    disp('!!!   vclass number of classes does not match !!!') ;
    disp(['!!!   K = ' num2str(K) '                                  !!!']) ;
    disp('!!!   Terminating Execution                   !!!') ; 
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    return ;
...
if vclass == 0 ;    %  No initial clustering given, so compute from data
  ...
  if twoMtype == 1 ;    %  Use Random Restarts for computation
    for k = 2:K ;
        ...
        [~, vindex] = SigClustKmeanRepPK(data,k,paramstruct) ;
            %  Ignore usual output of BestClass (Cluster Labelling)
        ClustIndData(k-1) = min(vindex) ;
  ...
  elseif twoMtype == 2 ;    %  Use PCA starts for computation
    for k = 2:K ;
        paramstruct = struct('maxstep',twoMsteps,  ...
                             'ioutplot',0, ...
                             'iscreenwrite',iscreenwritemost) ;
        [~, ClustIndData(k-1)]= SigClustKmeanFastPK(data,k,paramstruct) ;
            %  Ignore usual output of BestClass (Cluster Labelling)
    end ;
  ...
  ClustIndData = ClustIndPK(data,K,ClassFlags) ;
...
if iCovEst ~= 2 ;    %  then need to consider thresholding
  ...
  npix = d * n ;
  data = data - repmat(median(data,2),1,n) ; % median center data 
  ...
  else     %  Use Ming Yuan's Soft Thresholded, Constrained MLE
...
if  (iscreenwrite == 1)  ||  (iscreenwrite == 2)  ||  (iscreenwrite == 3)  ;
  if isempty(datastr) ;
    dispstr = 'MCSigClustPK:  Finished Estimation of Parameters for Gaussian simulation' ;
  else 
    dispstr = ['MCSigClustPK:  Finished Estimation of Parameters for Gaussian simulation, for ' datastr] ;
  end ;
  disp(dispstr) ;
end ;
  ...
  if vclass == 0 ;
      for k = 2:K ;
          if  iscreenwrite == 2  || (iscreenwrite == 3) ;
            disp(['      MCSigClustPK:  Working on sim ' num2str(isim) ' of ' num2str(nsim)]) ;
          end ;
          ...
          if twoMtype == 1 ;    %  Use Random Restarts for computation
            ...
            [~, vindex] = SigClustKmeanRepPK(mdatsim,k,paramstruct) ;
                %  Ignore usual output of BestClass (Cluster Labelling)
            ClustIndSim = min(vindex) ;
                %  Since SigClustKmeanRepPK returns full vector of Clust Indices
          ...
          elseif twoMtype == 2 ;    %  Use PCA starts for computation
            ...
            paramstruct = struct('maxstep',twoMsteps,  ...
                                 'ioutplot',0, ...
                                 'iscreenwrite',iscreenwriteall) ;
            [~, ClustIndSim]= SigClustKmeanFastPK(mdatsim,k,paramstruct) ;
                %  Ignore usual output of BestClass (Cluster Labelling)
          ...
          vSimIndex(isim,k-1) = ClustIndSim ;
      end ;
...
if  (iscreenwrite == 1)  ||  (iscreenwrite == 2)  ||  (iscreenwrite == 3)  ;
  if isempty(datastr) ;
    dispstr = 'MCSigClustPK:  Finished main simulation' ;
  else
    dispstr = ['MCSigClustPK:  Finished main simulation, for ' datastr] ;
  end ;
  disp(dispstr) ;
end ;
...
if ipvaltype ~= 2 ;    %  then compute Quantile based p-value
    if vclass == 0 ;
        for k = 2:K ;
            pvalQ(k-1) = cprobSM(vSimIndex(:,k-1),ClustIndData(k-1)) ;
        end ;
    else 
        pvalQ = cprobSM(vSimIndex,ClustIndData) ;
    end ;            
end ;
if ipvaltype ~= 1 ;    %  then compute Gaussian based p-value
    if vclass == 0 ;
        ...
        simsd = zeros(K-1,1) ;
        for k = 2:K ;
            simmean(k-1) = mean(vSimIndex(:,k-1)) ;
            simsd(k-1) = std(vSimIndex(:,k-1)) ;
            pvalZ(k-1) = normcdf((ClustIndData(k-1) - simmean(k-1)) / simsd(k-1)) ;
        end ;
    else
        simmean = mean(vSimIndex) ;
        simsd = std(vSimIndex) ;
        pvalZ = normcdf((ClustIndData - simmean) / simsd) ;
    end ;
end ;
...
if ipValplot == 1 ;    %  Then make p Value plot
  ...
  if vclass == 0 ;
    for k = 2:K ;
      vax = axisSM([vSimIndex(:,k-1); ClustIndData(k-1)]) ;
      if isempty(datastr) ;
        kdetitstr = [num2str(nsim) ' Gaussian Simulated Cluster Indices, K = ' num2str(k)] ;
      else 
      ...
      kdeparamstruct = struct('vxgrid',vax, ...
                              ...
                              'datovlaymax',0.6, ...
                              'iscreenwrite',iscreenwritemost) ;
      kdeSM(vSimIndex(:,k-1),kdeparamstruct) ;
      vax = axis ;
      hold on ;

        plot([ClustIndData(k-1); ClustIndData(k-1)],[vax(3); vax(4)],'Color','g') ;
        text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
             vax(3) + 0.9 * (vax(4) - vax(3)), ...
             ['Data Cluster Index = ' num2str(ClustIndData(k-1))],'Color','g') ;
        if ipvaltype ~= 2 ;    %  then write Quantile based p-value
          text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
               vax(3) + 0.8 * (vax(4) - vax(3)), ...
               ['p-val (Quantile) = ' num2str(pvalQ(k-1))],'Color','g') ;
        end;
        if ipvaltype ~= 1 ;    %  then write Gaussian based p-value
          text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
               vax(3) + 0.7 * (vax(4) - vax(3)), ...
               ['p-val (Gaussian) = ' num2str(pvalZ(k-1))],'Color','g') ;
          xgrid = linspace(vax(1),vax(2),401)' ;
          plot(xgrid,nmfSM(xgrid,simmean(k-1),simsd(k-1)^2,1),'k') ;
        end ;
        if varflag == 1 ;
          text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
               vax(3) + 0.55 * (vax(4) - vax(3)), ...
      ...
      if ~isempty(pValsavestr) ;
        orient landscape ;
          savestr = [pValsavestr '_K' num2str(k) '.ps'] ;
        print('-dpsc2',savestr) ;
      end ;
      ...
      hold on ;
        ...
        plot([ClustIndData; ClustIndData],[vax(3); vax(4)],'Color','g') ;
        text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
             vax(3) + 0.9 * (vax(4) - vax(3)), ...
             ['Data Cluster Index = ' num2str(ClustIndData)],'Color','g') ;
        if ipvaltype ~= 2 ;    %  then write Quantile based p-value
          text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
               vax(3) + 0.8 * (vax(4) - vax(3)), ...
               ['p-val (Quantile) = ' num2str(pvalQ)],'Color','g') ;
        end;
        if ipvaltype ~= 1 ;    %  then write Gaussian based p-value
          text(vax(1) + 0.5 * (vax(2) - vax(1)), ...
               vax(3) + 0.7 * (vax(4) - vax(3)), ...
               ['p-val (Gaussian) = ' num2str(pvalZ)],'Color','g') ;
          xgrid = linspace(vax(1),vax(2),401)' ;
...
if  (iscreenwrite == 1)  ||  (iscreenwrite == 2)  ||  (iscreenwrite == 3)  ;
  if isempty(datastr) ;
    if ipvaltype == 1 ;    %  then write Quantile based p-value
      dispstr1 = 'MCSigClustPK:  Finished with Qp-values:' ;
    elseif ipvaltype == 2 ;    %  then write Gaussian based p-value
      dispstr1 = 'MCSigClustPK:  Finished with Zp-values:' ;
    else     %  then write both p-values
      dispstr1 = 'MCSigClustPK:  Finished with Qp-values & Zp-values:' ;
    end ;
  else 
    if ipvaltype == 1 ;    %  then write Quantile based p-value
      dispstr1 = ['MCSigClustPK:  Finished, for ' datastr ...
                                      ', with Qp-values:'] ;
    elseif ipvaltype == 2 ;    %  then write Gaussian based p-value
      dispstr1 = ['MCSigClustPK:  Finished, for ' datastr ...
                                      ', with Zp-values:'] ;
  ...
  if ~(ipvaltype == 2) ;
      disp(['Qp-values = ' num2str(pvalQ')]) ;
  end ;
  if ~(ipvaltype == 1) ;
      disp(['Zp-values = ' num2str(pvalZ')]) ;
  end ;
