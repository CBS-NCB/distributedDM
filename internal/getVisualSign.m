function [VFS, VFS_thr] = getVisualSign(retinotopyData)
  %params.resFac = 0.8;
  params.resFac = 0.8;
  trn = 1;
  ang0 = angle(retinotopyData.CmplxMaps{trn,1}{1});
  ang2 = angle(retinotopyData.CmplxMaps{trn,1}{2});
  ang1 = angle(retinotopyData.CmplxMaps{trn,2}{1});
  ang3 = angle(retinotopyData.CmplxMaps{trn,2}{2});
  ...
  % radians to degrees
  map_hor   = (map_hor  /(2*pi)) * retinotopyData.visual_field(1);
  map_vert  = (map_vert /(2*pi)) * retinotopyData.visual_field(2);
